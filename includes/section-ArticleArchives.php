...
<p>
        ...
        <!-- <?php echo the_terms($postID, 'tags', '<span><i class="fas fa-tag"></i>', ' | ', '</span>'); ?> -->
        Tags: 
        <?php $articleTerms = wp_get_post_terms($post->ID, 'tags');
                if ($articleTerms) {
                        $tagsArray = array();
                        foreach ($articleTerms as $articleTerm) {
                                $tagsArray[] = '<span><i class="fas fa-tag"></i><a class="' .$articleTerm->slug .'" href="' .get_term_link( $articleTerm->slug, 'tags') .'">' .$articleTerm->name .'</a></span>';
                        }
                        echo join( '&nbsp;&nbsp;&nbsp;&nbsp;', $tagsArray );
                };
        ?>
